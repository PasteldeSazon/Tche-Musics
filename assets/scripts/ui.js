    ...
    .map(
      (musica) => `
            <div class="card" data-id="${musica.id}">
                ...
                <h2>${musica.titulo}</h2>
                <p>${musica.autor}</p>
                                <p>Popularidade: ${musica.popularidade.toLocaleString("pt-BR")}</p>                

                <button
                    type="button"
                    class="botao-coracao ${musica.favorita ? "favorito" : ""}"
                    data-id="${musica.id}"
                    aria-label="Favoritar ${musica.titulo}"
                    aria-pressed="${musica.favorita}"
                >
                    ${
                      musica.favorita
                        ? `
                          ...
                          `
                        : `
                            <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-heart" viewBox="0 0 16 16" aria-hidden="true">
...
export default renderizarMusicas;
